...
// import { onAuthStateChanged, signInWithCustomToken, signInWithPopup, signInAnonymously, signOut } from 'firebase/auth';
import { Menu, Sun, Moon, Download, Plus } from 'lucide-react';
...
import SessionUpdatesView from './views/SessionUpdatesView';
import SessionUpdateModal from './components/SessionUpdateModal';
...
const App = () => {
  ...
  const [newFaculty, setNewFaculty] = useState({ name: '', mobile: '', email: '', designation: '', subjects: [] });
  const [assignments, setAssignments] = useState([]);
  const [isAddingAssignment, setIsAddingAssignment] = useState(false);
  const [sessionUpdates, setSessionUpdates] = useState([]);
  const [isAddingSession, setIsAddingSession] = useState(false);
  const [editingSession, setEditingSession] = useState(null);
  const [currentSessionData, setCurrentSessionData] = useState({ facultyId: '', course: '', batchSlot: '', topicsCovered: '', date: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      if (u) {
        // Security Check: Is this email on our list?
        if (ALLOWED_EMAILS.includes(u.email)) {
          ...
          await signOut(auth);
          setUser(null);
          alert("Access Denied: This email is not registered for the ERP.");
        }
      } else {
        setUser(null);
      }
  ...
  const saveStudent = async () => {
    try {
      ...
      if (editingStudent) {
        ...
        const updates = {
          ...newStudent,
          email: newStudent.email || '', // Syncing the student email field on update
          joiningDate: Timestamp.fromDate(new Date(newStudent.joiningDate)),
          assignedFacultyIds: newStudent.assignedFacultyIds || [],
          assignedFacultyNames: newStudent.assignedFacultyNames || [],
          courses: newStudent.courses || [],
          course: newStudent.courses && newStudent.courses.length > 0 ? newStudent.courses.map(c => typeof c === 'object' ? c.name : c).join(', ') : 'N/A'
        };
        ...
        await addDoc(studentsCollection, {
          ...newStudent,
          email: newStudent.email || '', // Syncing the student email field on creation
          joiningDate: Timestamp.fromDate(new Date(newStudent.joiningDate)),
      ...
      setNewStudent({ name: '', mobile: '', email: '', course: '', courses: [], batchFrom: '', batchTo: '', joiningDate: '', status: 'Active', totalFee: '', discount: '' });
    } catch (e) { console.error(e); }
  };
  ...
  const startEdit = (lead) => {
    ...
    setNewLead({
      ...
      // Load current multiple courses or fallback to array containing original single course
      courses: lead.courses || (lead.course ? [lead.course] : [])
    });
  ...
  const closeLeadModal = () => {
    ...
    setEditingLead(null);
    setNewLead({ name: '', phone: '', priority: 'Medium', remarks: '', source: PLATFORMS[0], courses: [] });
  };
  ...
  const editStudent = (student) => {
    ...
    setNewStudent({
      ...
      email: student.email || '', // Prepopulate student email in state
      batchTo: student.batchTo || '',
      batchFrom: student.batchFrom || '',
      status: student.status || 'Active',
      totalFee: student.totalFee || '',
      discount: student.discount || '',
      joiningDate: student.joiningDate?.seconds ? new Date(student.joiningDate.seconds * 1000).toISOString().split('T')[0] : '',
      assignedFacultyIds: student.assignedFacultyIds || [],
      assignedFacultyNames: student.assignedFacultyNames || [],
      courses: (student.courses || []).map(c => typeof c === 'object' ? c.name : c)
    });
  ...
  const handleExecuteConversion = async (additionalDetails) => {
    ...
    try {
      ...
      const studentId = String(conversionLead.id); // Unique student ID based on lead ID and timestamp
      ...
      const leadMobileNumber = conversionLead.mobile || conversionLead.phone || conversionLead.contact || 'N/A';

      // 1. Compile the student registration profile payload
      // Assemble the student profile inside handleExecuteConversion
      const studentPayload = {
        id: studentId,
        leadId: String(conversionLead.id),
        name: String(conversionLead.name),
        mobile: String(leadMobileNumber),

        // PASS THE CONVERTED MULTI-COURSE ARRAY INSTEAD OF A SINGULAR VALUE
        courses: additionalDetails.courses || [],
        // Keep course string present as backward compatibility for older cards
        course: additionalDetails.courses && additionalDetails.courses.length > 0 ? additionalDetails.courses[0] : 'N/A',
        ...
        createdAt: serverTimestamp(),
        assignedFacultyIds: additionalDetails.assignedFacultyIds || [],
        assignedFacultyNames: additionalDetails.assignedFacultyNames || [],
        statusHistory: {
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(l => {
      // Compatibility Fallback array computation
      const leadCourses = l.courses || (l.course ? [l.course] : []);

      const matchesSearch = l.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        l.phone?.includes(searchQuery) ||
        leadCourses.some(c => c.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      const matchesCourse = courseFilter === 'All' || leadCourses.includes(courseFilter);
  ...
  const filteredStudents = useMemo(() => {
    return students.filter(student => {
      ...
      // Fallback fallback to read either array courses or historical course string
      ...
      const matchesSearch = student.name?.toLowerCase().includes(search) ||
        ...
        studentCourses.some(c => c.toLowerCase().includes(search)) ||
        student.batchFrom?.toLowerCase().includes(search) ||
        student.batchTo?.toLowerCase().includes(search) ||
  ...
  useEffect(() => {
    if (!user) return;
    const facultyCollection = collection(db, 'artifacts', appId, 'public', 'data', 'faculty');
    return onSnapshot(facultyCollection, (snapshot) => {
      setFaculty(snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })));
    }, (err) => console.error("Faculty Sync Error:", err));
  }, [user]);
  const handleSaveFaculty = async (e) => {
    e.preventDefault();
    try {
      const { doc, addDoc, collection } = await import('firebase/firestore');
      if (editingFaculty) {
        const facDocRef = doc(db, 'artifacts', appId, 'public', 'data', 'faculty', editingFaculty);
        await updateDoc(facDocRef, { ...newFaculty });
      } else {
        const facultyCollection = collection(db, 'artifacts', appId, 'public', 'data', 'faculty');
      ...
      console.error("Faculty operational execution matrix crash:", err);
    }
  };
  ...
  const startEditFaculty = (member) => {
    ...
    setNewFaculty({
      ...
      photoUrl: member.photoUrl || '', // <-- ADD THIS LINE
  ...
  const deleteFaculty = async (id) => {
    if (!window.confirm("Are you sure you want to permanently remove this instructor account file?")) return;
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'faculty', id));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const closeFacultyModal = () => {
    ...
    setNewFaculty({ name: '', mobile: '', email: '', designation: '', photoUrl: '', subjects: [] });
  };

  // Assignment Section Useffect for firebase snapshot
  useEffect(() => {
    if (!user) return;
    const assignmentsCollection = collection(db, 'artifacts', appId, 'public', 'data', 'assignments');
    return onSnapshot(assignmentsCollection, (snapshot) => {
      setAssignments(snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })));
    }, (err) => console.error("Assignment Stream Exception Fail:", err));
  }, [user]);
  ...
  const handleSaveAssignment = async (assignmentPayload) => {
    try {
      ...
      console.error("Critical Assignment Write Exception logged:", e);
    }
  };

  const updateSubmissionStatus = async (assignmentId, studentId, nextStatus) => {
    try {
      const assignmentDocRef = doc(db, 'artifacts', appId, 'public', 'data', 'assignments', assignmentId);
      await updateDoc(assignmentDocRef, {
        [`submissions.${studentId}`]: nextStatus
      });
    } catch (e) {
      console.error("Submission Status Patch Sync Failed:", e);
    }
  };

  const deleteAssignment = async (id) => {
    if (!window.confirm("Are you sure you want to completely archive and remove this assignment task record?")) return;
    try {
  ...
  useEffect(() => {
    if (!user) return;
    const sessionsCollection = collection(db, 'artifacts', appId, 'public', 'data', 'session_updates');
    return onSnapshot(sessionsCollection, (snapshot) => {
      setSessionUpdates(snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })).sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)));
    }, (err) => console.error("Session Update Sync Fallback Fail:", err));
  }, [user]);

  // HANDLNG SESSION UPDATE CREATION
  const handleSaveSessionUpdate = async (sessionPayload) => {
    try {
      const { collection, addDoc, doc, updateDoc, serverTimestamp } = await import('firebase/firestore');

      const formattedPayload = {
        facultyId: sessionPayload.facultyId,         // e.g., "fac_123" (Faculty A)
        facultyName: sessionPayload.facultyName,     // e.g., "Prof. Shyam"
        course: sessionPayload.course,               // e.g., "Python" (Not the whole array, just this session's course!)
        batchSlot: sessionPayload.batchSlot,         // e.g., "10:00 AM - 11:00 AM"
        topicsCovered: sessionPayload.topicsCovered,
        date: sessionPayload.date,
        assignedStudentIds: sessionPayload.assignedStudentIds || [],
        assignedStudentNames: sessionPayload.assignedStudentNames || []
      };

      if (editingSession) {
        const sessionDocRef = doc(db, 'artifacts', appId, 'public', 'data', 'session_updates', editingSession);
        await updateDoc(sessionDocRef, formattedPayload);
      } else {
        const sessionsCollection = collection(db, 'artifacts', appId, 'public', 'data', 'session_updates');
      ...
      console.error("Session log generation failure:", err);
    }
  };

  // HANDLING SESSION UPDATE EDITING
  const startEditSession = (session) => {
    setEditingSession(session.id);
    // Inside your startEditSession handler in App.js:
    setCurrentSessionData({
      facultyId: session.facultyId || '',
      course: session.course || '',
      batchSlot: session.batchSlot || '',
      topicsCovered: session.topicsCovered || '',
      date: session.date || '',
      assignedStudentIds: session.assignedStudentIds || [], // Array Init
      assignedStudentNames: session.assignedStudentNames || [] // Array Init
    });

    // Inside your closeSessionModal handler in App.js:
    // setCurrentSessionData({ facultyId: '', course: '', batchSlot: '', topicsCovered: '', date: '', assignedStudentIds: [], assignedStudentNames: [] });
  ...
  const closeSessionModal = () => {
    ...
    setCurrentSessionData({ facultyId: '', course: '', batchSlot: '', topicsCovered: '', date: '' });
  };
  // HANDLING SESSION UPDATE DELETION

  const deleteSessionUpdate = async (id) => {
    if (!window.confirm("Are you sure you want to remove this session entry log from the archive timeline?")) return;
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'session_updates', id));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  if (authLoading) return <div className="h-screen bg-white dark:bg-gray-950 flex items-center justify-center text-indigo-500 font-bold">Initializing ERP...</div>;
    ...
    <div className="flex h-screen bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-800 text-gray-900 dark:text-gray-100 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          {activeTab === 'dashboard' && <DashboardView stats={stats} globalMonth={globalMonth} setGlobalMonth={setGlobalMonth} monthOptions={monthOptions} students={students} setSelectedStatuses={setSelectedStatuses} setActiveTab={setActiveTab} setSelectedStatus={setSelectedStatus} inquiries={leads} />}
          ...
          {activeTab === 'faculty' && <FacultyView faculty={faculty} editFaculty={startEditFaculty} deleteFaculty={deleteFaculty} setIsAddingFaculty={setIsAddingFaculty} students={students} />}
          {activeTab === 'assignments' && <AssignmentsView assignments={assignments} students={students} deleteAssignment={deleteAssignment} setIsAddingAssignment={setIsAddingAssignment} updateSubmissionStatus={updateSubmissionStatus} />}
          {activeTab === 'session-updates' && <SessionUpdatesView sessionUpdates={sessionUpdates} faculty={faculty} setIsAddingSession={setIsAddingSession} deleteSessionUpdate={deleteSessionUpdate} startEditSession={startEditSession} />}
        </div>
      </main>
      ...
      {showStudentModal && <StudentModal editingStudent={editingStudent} setShowStudentModal={setShowStudentModal} setEditingStudent={setEditingStudent} newStudent={newStudent} setNewStudent={setNewStudent} saveStudent={saveStudent} faculty={faculty} />}
      ...
      {isAddingSession && (
        <SessionUpdateModal
          ...
          handleSaveSession={handleSaveSessionUpdate}
          faculty={faculty}
          students={students}
