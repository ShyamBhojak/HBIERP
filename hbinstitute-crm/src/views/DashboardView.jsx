...
const DashboardView = ({ stats, globalMonth, setGlobalMonth, monthOptions, students, setSelectedStatuses, setActiveTab, setSelectedStatus, inquiries = [] }) => {
  // 1. Calculate Total Active Students Enrolled in the Selected Global Month
  const enrolledThisMonthCount = useMemo(() => {
    return students.filter(student => {
      // Match students whose enrollment track belongs to the active dashboard month
      return student.status === 'Active' && student.month === globalMonth;
    }).length;
  }, [students, globalMonth]);

  // 2. Calculate the updated Conversion Rate dynamically for the Selected Global Month
  const currentMonthConversionRate = useMemo(() => {
    // Total inquiries (leads) in the selected global month
    const currentMonthInquiries = inquiries.filter(lead => lead.month === globalMonth).length;

    if (currentMonthInquiries === 0) return 0;

    // (Total Enrolled in Selected Month / Total Inquiries in Selected Month) * 100
    return ((enrolledThisMonthCount / currentMonthInquiries) * 100).toFixed(1);
  }, [enrolledThisMonthCount, inquiries, globalMonth]);
  ...
  const statCards = [
    ...
    {
      label: 'Enrolled Today', value: enrolledThisMonthCount, icon: CheckCircle2, color: 'text-indigo-400', bg: 'bg-indigo-400/5',
    ...
    { label: 'Conversion rate', value: `${currentMonthConversionRate}%`, icon: TrendingUp, color: 'text-green-400', bg: 'bg-green-400/5' }
  ];
