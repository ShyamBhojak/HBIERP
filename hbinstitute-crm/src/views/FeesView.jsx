  ...
  const ledgerData = useMemo(() => {
    return students
      ...
      .map(student => {
        ...
        return { ...student, totalFee, paid, balance: totalFee - paid, paymentHistory: history };
      });
  }, [students, studentPayments, feesRecords, searchQuery, effectiveMonth]);

  const downloadReceiptPDF = (student, currentTx) => {
    ...
    const htmlInvoiceContent = `
      <html>
        <head>
          <title>${cleanStudentName}_rcpt</title>
          <style>
            ...
            * { box-sizing: border-box; font-family: 'Inter', sans-serif; margin: 0; padding: 0; }
            ...
            .summary-box { background: #f8fafc; border-radius: 18px; padding: 20px; width: 320px; }
            ...
            .signatory-line { border-top: 1px solid #cbd5e1; margin-top: 5px; padding-top: 6px; font-size: 10px; font-weight: 800; uppercase; tracking: 0.05em; color: #475569; }
          ...
          <div class="receipt-frame">
            ...
            <table class="table-box">
              ...
              <tbody>
                ${studentHistory.map(tx => `
                  <tr style="${tx.id === currentTx.id ? 'background-color: #f1f5f9;' : ''}">
                    <td>${tx.date ? tx.date.split('-').reverse().join('/') : 'N/A'}</td>
                    <td>${tx.mode || 'Cash'}</td>
                    <td style="color:#64748b; font-size:12px;">${tx.remarks || 'Installment Entry'} ${tx.id === currentTx.id ? '(Current)' : ''}</td>
            ...
            <div class="closing-block">
              ...
              <div class="summary-box">
                <div class="summary-row"><span>Total Course Cost</span><span>₹${student.totalFee.toLocaleString('en-IN')}.00</span></div>
                <div class="summary-row"><span>Total Paid Collections</span><span style="color:#16a34a; font-weight:700;">₹${student.paid.toLocaleString('en-IN')}.00</span></div>
                <div class="summary-row total-row"><span>Outstanding Dues</span><span>₹${student.balance.toLocaleString('en-IN')}.00</span></div>
              </div>
            </div>
                              ...
                              {student.paymentHistory.map((tx, index) => (
                                <div key={tx.id || index} className="flex justify-between items-center bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800/60 rounded-xl p-4 text-xs">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); downloadReceiptPDF(student, tx); }} className="p-2 bg-gray-100 hover:bg-indigo-600 text-gray-500 hover:text-white rounded-lg transition-all" title="Download PDF Receipt"><Download size={14} /></button>
                                  </div>
                                </div>
                              ))}
