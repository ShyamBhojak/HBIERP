...
const FeesView = ({ students, feesRecords, saveFeePayment, globalMonth }) => {
  ...
  const financeStats = useMemo(() => {
    ...
    students.forEach(s => {
      if (s.status === 'Active' || s.status === 'Completed') {
        ...
        const allowedDiscount = Number(s.discount) || 0; // <-- Parse allowed discount
  ...
  const ledgerData = useMemo(() => {
    return students
      ...
      .map(student => {
        ...
        const discount = Number(student.discount) || 0; // <-- Fetch profile discount
  ...
  const downloadReceiptPDF = (student, currentTx) => {
    ...
    const paid = studentPayments[student.id] || 0;
    const balance = totalFee - discount - paid;
